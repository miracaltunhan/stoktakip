...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Yeni Stok Hareketi</h1>
    </div>
        ...
        <form action="{{ route('stock-movements.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="type" id="type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    <option value="in" {{ old('type') == 'in' ? 'selected' : '' }}>Giriş</option>
                    <option value="out" {{ old('type') == 'out' ? 'selected' : '' }}>Çıkış</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
